...
struct SearchContext* __stdcall create(const struct Point* points_begin, const struct Point* points_end) {
    ...
    while (begin_copy != points_end) {
        size++;
        if (max_rank < begin_copy->rank) max_rank = begin_copy->rank;
        begin_copy++;
    } //
    
    sc->size = size;
    sc->max_rank = max_rank;
...
int32_t __stdcall search(struct SearchContext* sc, const struct Rect rect, const int32_t count, struct Point* out_points) {
    ...
    // point_radix_rank_heap(sc->points, right - left + 1, sc->max_rank);
    // int i = left;
    // int found = 0;
    ...
    for (int i = left; i < right + 1; i++) {
        if (sc->points[i].y >= rect.ly && sc->points[i].y <= rect.hy) {
            ...
            found_size++;
            if (max_rank < sc->points[i].rank) max_rank = sc->points[i].rank;
        }
    }
    ...
    if (count < found_size) {
        ...
        // }
        // count_sort_stack(rectangle_points, 0, found_size - 1);
        // count_sort_128(rectangle_points, count);
        point_radix_rank_heap(rectangle_points, found_size, max_rank);
        ...
        point_radix_rank_stack(rectangle_points, found_size, max_rank);
