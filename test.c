...
Point random_point(int id, int rank_max, float coord_max) {
    Point point;
    point.id = (int8_t)id;
...
Rect random_rect(float coord_max, float rect_max_width) {
    Rect rect;
...
void test_merge_sort(Point* points, int size, int n) {
    printf("\nMerge sort test::\n");
    printf("Before merge\n");
...
void test_merge(Point* points, int left, int middle, int right) {
    printf("Merge test::\n");
    Point* buffer = (Point*)malloc((right - middle + 1) * sizeof(Point));
    point_merge(points, buffer, right, middle, left, point_less_than_x);
    print_points(&points[left], right - left + 1);
}

void test_closest_greater_index(Point* points, int size, float value) {
...
void test_closest_lesser_index(Point* points, int size, float value) {
...
void print_search_result(Rect rect, int32_t count, Point* out_points) {
    printf("\nSearch results::\n");
...
int main(int argc, char* argv[]) {
    ...
    Point* input_points = (Point*)malloc(POINT_COUNT * sizeof(Point));
    ...
    start = time(NULL);
    SearchContext* sc = create(&input_points[0], &input_points[POINT_COUNT]);
    printf("Time: %ld\n\n", time(NULL) - start);
    ...
    start = time(NULL);
    Point* out_points = (Point*)malloc(RESULT_COUNT * sizeof(Point));
    int count = 0;
    for (int i = 0; i < RECT_COUNT; i++) {
